...
class CacheLine{
	...
	public void statModifier(int state){
		...
		if(state == 0 ){
			this.valid = false ; this.empty = false ; 
		}
		if(state==1 ){
			this.valid=true ;this.dirty = false;
		}
		if(state==2){
...
public class ConventionalHPC {
	...
	//All The Initial Testing Scrap 
	//System.out.println(fourthTrace.get(fourthTrace.size()-1));
	
	/*String test[]=new String[10];
	test[1]=firstTrace.get(0);
	String Array[]=test[1].split(" ");
	Array[0]*/
	
	/*insert(modelCache,tagS,indexS,offsetS);
	...
	tagS="01111010";
	insert(modelCache,tagS,indexS,offsetS);
	System.out.println(search(modelCache, tagS, indexS, offsetS));
	*/
	...
	{	
		...
		String memAdd_p1 = tempTokens_p1[1];
		 ...
		 offsetS = memAdd_p1.substring(15);
		 if(instruc_p1=='R'){
			if(search(p1.cache, tagS,indexS,offsetS)>=0){
				total_reads++;
			}
			else{
				insert(p1.cache, tagS, indexS, offsetS);
	...
	*/
	//System.out.println("total reads = " +total_read_miss + "total_writes = "+ total_write_miss);
